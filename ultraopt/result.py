...
class Run(object):
    ...
    def __init__(self, config_id, budget, loss, info, timestamps, error_logs):
    ...
    def __repr__(self):
        return ( \
                    ...
                    "loss: %s\n" % self.loss + \
                    "timestamps: {submitted} (submitted), {started} (started), {finished} (finished)\n".format(
                        **self.timestamps) + \
                    "info: %s\n" % self.info
        )
...
def logged_results_to_HBS_result(directory):
    ...
    Returns
    -------
    ambo.core.result.Result: :object:
        TODO
    ...
    with open(os.path.join(directory, 'results.json')) as fh:
        for line in fh:
            config_id, budget, timestamps, result, exception = json.loads(line)

            id = tuple(config_id)

            data[id].timestamps[budget] = timestamps
            data[id].results[budget] = result
            data[id].exceptions[budget] = exception

            budget_set.add(budget)
            time_ref = min(time_ref, timestamps['submitted'])
    ...
    def get_incumbent_trajectory(self, all_budgets=True, bigger_is_better=True, non_decreasing_budget=True):
        ...
        for r in all_runs:
            ...
            if new_incumbent:
                ...
                return_dict['times_finished'].append(r.timestamps['finished'])
        ...
        for k, v in new_dict.items():
            for kk, vv in v.timestamps.items():
                for kkk, vvv in vv.items():
                    new_dict[k].timestamps[kk][kkk] = vvv - self.HB_config['time_ref']
